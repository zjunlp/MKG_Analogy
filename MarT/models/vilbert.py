...
class BertSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, sep_idx=None):
...
class BertAttention(nn.Module):
    ...
    def forward(self, input_tensor, attention_mask ,sep_idx=None):
        self_output, attention_probs = self.self(input_tensor, attention_mask, sep_idx=sep_idx)
...
class BertLayer(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask, sep_idx=None):
        attention_output, attention_probs = self.attention(
            hidden_states, attention_mask, sep_idx=sep_idx
        )
            ...
            for idx in range(t_start, self.fixed_t_layer):
                with torch.no_grad():
                    txt_embedding, txt_attention_probs = self.layer[idx](
                        txt_embedding, txt_attention_mask, sep_idx=sep_idx,
                    )
...
class VilBertForMaskLM(BertPreTrainedModel):
    def __init__(self, config):
        super(VilBertForMaskLM, self).__init__(config)
    ...
    def forward(
        ...
        prediction_scores, trans_hidden_states = self.cls(sequence_output_t)
        ...
        if not return_dict:
            output = (prediction_scores,) + (all_attention_mask,)
