...
class FlavaImageEmbeddings(nn.Module):
    ...
    def forward(
        ...
        # rewrite
        batch_size, num_ent, num_channels, height, width = pixel_values.shape
        head_embeddings = self.patch_embeddings(pixel_values[:, 0], interpolate_pos_encoding=interpolate_pos_encoding)
        tail_embeddings = self.patch_embeddings(pixel_values[:, 1], interpolate_pos_encoding=interpolate_pos_encoding)

        batch_size, seq_len, _ = head_embeddings.size()
        ...
        embeddings = torch.cat((cls_tokens, head_embeddings), dim=1)    # bsz, 197, 768
        ...
        if interpolate_pos_encoding:
            ...
            tail_embeddings = tail_embeddings + self.position_embeddings[:, :tail_embeddings.shape[1]]

        embeddings = torch.cat((embeddings, tail_embeddings), dim=1)    # bsz, 393, 768
    ...
    def forward(
        ...
        if sep_idx is not None:
            for i, idx in enumerate(sep_idx):
                # example to answer
                attention_scores[i, :, 1:idx[2], idx[2]:] = torch.clamp(self.adaptive_weight[0], 0, 0.5) * attention_scores[i, :, 1:idx[2], idx[2]:].clone()
                # answer to example
...
class FlavaAttention(nn.Module):
    ...
    def forward(
        ...
        self_outputs = self.attention(
            hidden_states, attention_mask=attention_mask, head_mask=head_mask, output_attentions=output_attentions, sep_idx=sep_idx,
        )
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(hidden_states, attention_mask, layer_head_mask, output_attentions, sep_idx=sep_idx)
...
class FlavaMaskedPredictionHead(nn.Module):
    ...
    def forward(self, x):
        trans_hidden_states = self.transform(x)
        x = self.decoder(trans_hidden_states)
        return x, trans_hidden_states
...
class FlavaForMaskedLM(FlavaPreTrainedModel):

    def __init__(self, config: FlavaConfig):
        super().__init__(config)
        
        self.flava = FlavaModel(config)
        self.cls = FlavaMaskedPredictionHead(config.text_config)
        
        # Initialize weights and apply final processing
        self.post_init()
        
    def get_input_embeddings(self) -> PatchEmbeddings:
        return self.flava.text_model.embeddings.word_embeddings
    
    def set_input_embeddings(self, value: nn.Module):
        self.flava.text_model.embeddings.word_embeddings = value
    ...
    def forward(
        ...
        text_masked_embeddings = outputs.text_embeddings
        multimodal_embeddings = outputs.multimodal_embeddings
        sequence_for_text = multimodal_embeddings[:, -text_masked_embeddings.size(1) :, :]
        prediction_scores, trans_hidden_states = self.cls(sequence_for_text)
