...
class ViltSelfAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False, sep_idx=None, img_length=None):
        ...
        if sep_idx is not None:
             for i, idx in enumerate(sep_idx):
                # example to answer
                attention_scores[i, :, img_length+1:idx[2], idx[2]:] = torch.clamp(self.adaptive_weight[0], 0, 0.5) * attention_scores[i, :, img_length+1:idx[2], idx[2]:].clone()
                # answer to example
...
class ViltAttention(nn.Module):
    ...
    def forward(self, hidden_states, attention_mask=None, head_mask=None, output_attentions=False, sep_idx=None, img_length=None):
        self_outputs = self.attention(hidden_states, attention_mask, head_mask, output_attentions, sep_idx=sep_idx, img_length=img_length,)
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.layer):
            ...
            if self.gradient_checkpointing and self.training:
                ...
                layer_outputs = layer_module(hidden_states, attention_mask, layer_head_mask, output_attentions, sep_idx=sep_idx, img_length=img_length,)
...
class ViltModel(ViltPreTrainedModel):
    ...
    def forward(
        ...
        img_length = embedding_output.shape[1] - input_ids.shape[1]
        sep_idx += img_length
...
class ViltForMaskedLM(ViltPreTrainedModel):
    ...
    def forward(
        ...
        mlm_logits, trans_hidden_states = self.mlm_score(text_features)
